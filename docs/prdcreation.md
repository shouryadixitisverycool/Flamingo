...
i wanna overhaul the playlist feature. i've shown the images of the new UI how i roughly want it to look. i wanna also have a search bar, similar to the ones we've implemented so far, except this one is hidden initially and only pops up if i swipe down, for me to search the playlist. then i need to overhaul the 3-dot menu on the top-right of the playlist page, make it similar visually to the 3-dot menu in the nowplaying page, having features like "sort by" (where the sorting options will lie which already exist), "edit playlist" (refer to the screenshot provided), "pin playlist", "add to a playlist" (which opens our playlist picker we've made we'll reuse it), "play next" (which adds it to the queue), "delete playlist" (which will give a warning then delete the playlist).
